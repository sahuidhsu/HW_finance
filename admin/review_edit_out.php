...
if (isset($_POST["submit"])) {
    try {
        $sql = $conn->prepare("UPDATE out_fee SET date = :date, project_id = :project_id, fee_id = :fee_id, amount = :amount, comment = :comment, valid = :valid WHERE id = :id;");
        $sql->execute([
            "date" => $_POST["date"],
...
<head>
    <title>
        编辑支出费用 - <?php echo $site_name; ?>
    </title>
</head>
<body>
<div class='container' style='margin-top: 2%;'>
    <div class='card border-dark'>
        <h4 class='card-header bg-primary text-white text-center'>编辑费用 - 支出</h4>
        <form action='' method='post' style='margin: 20px;'>
            ...
            <div class='input-group mb-3'>
                ...
                <select class='form-select' name='fee_id'>
                    <?php
                    $sql2 = $conn->prepare("SELECT * FROM fee;");
                    $sql2->execute();
                    $result3 = $sql2->fetchAll();
                    foreach ($result3 as $row) {
                        echo "<option value='" . $row["id"] . "' " . ($row["id"] == $result["fee_id"] ? "selected" : "") . ">" . $row["name"] . "</option>";
                    }
                    ?>
                </select>
            </div>
